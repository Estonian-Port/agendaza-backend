...
interface PagoRepository : CrudRepository<Pago, Long>{
    ...
    @Query("SELECT new com.estonianport.agendaza.dto.PagoDTO(p.id, p.monto, p.concepto, 0,ev.codigo, p.medioDePago," +
            "ev.nombre, p.fecha, ev.empresa.id, p.encargado.id) FROM Pago p LEFT JOIN p.evento ev WHERE p.evento.id = :idEvento AND p.fechaBaja IS NULL ORDER BY p.fecha DESC")
