...
interface ExtraRepository : CrudRepository<Extra, Long>{
    ...
    fun cantidadExtrasCateringFilter(id : Long, buscar: String) : Int
    ...
    @Query("""SELECT new com.estonianport.agendaza.dto.ExtraDTO(e.id, e.nombre, e.tipoExtra)
        FROM Extra e
        WHERE e.fechaBaja IS NULL
          AND (e.tipoExtra = TipoExtra.EVENTO
               OR e.tipoExtra = TipoExtra.VARIABLE_EVENTO)
          AND e.id NOT IN (
    ...
    @Query("""SELECT new com.estonianport.agendaza.dto.ExtraDTO(e.id, e.nombre, e.tipoExtra)
        FROM Extra e
        WHERE e.fechaBaja IS NULL
          AND (e.tipoExtra = TipoExtra.TIPO_CATERING
               OR e.tipoExtra = TipoExtra.VARIABLE_CATERING)
          AND e.id NOT IN (
              SELECT e2.id FROM Extra e2
              JOIN e2.listaEmpresa emp
              WHERE emp.id = :empresaId
